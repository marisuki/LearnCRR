...
from lineartree2._criterion import SCORING
from lineartree2._criterion import mse, rmse, mae, poisson
from lineartree2._criterion import hamming, crossentropy
...
def _parallel_binning_fit(split_feat, _self, X, y,
    ...
    for col, _bin in zip(split_feat, bins):
        if col == 1: 
            ...
            #_bins = _bin
            # binary
            #_bins = [(min(_bin)+max(_bin))/2.0]
            # triary
            #_bins = [(2*min(_bin)+max(_bin))/3.0, (min(_bin)+2*max(_bin))/3.0]
            # random
...
def _ext_model_fitting(X, y, model_id, col, q, _self):
    ...
    return np.sqrt(mean_squared_error(lp + lm, y[~mask])), np.sqrt(mean_squared_error(rp + rm, y[mask])), lm, rm, model_id
